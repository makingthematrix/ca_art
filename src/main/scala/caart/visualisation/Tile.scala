...
import javafx.scene.input.{DragEvent, MouseButton, MouseEvent}
...
final case class Tile[C <: AutomatonCell[C]](cell:         () => C,
  ...
  def initialize(): Unit = {
    ...
    tile.setOnDragOver { (ev: DragEvent) => onUserEvent ! UserEvent(pos, UserEventType.MouseDrag) }
    tile.setOnDragDropped { (ev: DragEvent) => onUserEvent ! UserEvent(pos, UserEventType.MouseDragFinished) }
  }
}
