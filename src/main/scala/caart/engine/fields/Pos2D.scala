...
object Pos2D {
  ...
  def apply(dim: Int): Vector[Pos2D] = (0 until dim).flatMap(x => (0 until dim).map(Pos2D(x, _))).toVector

  // Returns all positions for a square board between two positions (inclusive)    
  def range(p1: Pos2D, p2: Pos2D): Vector[Pos2D] = {
    val xfrom = math.min(p1.x, p2.x)
    val yfrom = math.min(p1.y, p2.y)
    val xto = math.max(p1.x, p2.x)
    val yto = math.max(p1.y, p2.y)

    (xfrom to xto).flatMap(x => (yfrom to yto).map(Pos2D(x, _))).toVector
  }
