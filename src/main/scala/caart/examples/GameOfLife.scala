...
final case class GameOfLife(override val pos: Pos2D,
                            ...
                            life: Boolean = false) extends Cell.NoGlobal[GameOfLife] {
  override type CE = GameOfLife.FlipCell.type

  override def updateFromEvents(events: Iterable[GameOfLife.FlipCell.type]): Option[GameOfLife] =
    if (events.size % 2 == 1) Some(copy(life = !life)) else None

  override def selfUpdate: Option[GameOfLife] = auto.moore(pos).values.count(_.life) match {
